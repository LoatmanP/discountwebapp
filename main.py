...
#     return avg
    ...
    if option == 'Exponential':
        st.markdown('{} decision makers have *dynamically consistent* preferences, meaning preferences for choices will not change as those choice items move through time'.format(option))
    ...
    if (Average(accuracy_list) * 100) > 80:
        st.subheader('Your intertemporal preferences are well-described by the {} Discounting model'.format(option))
    elif (Average(accuracy_list) * 100) > 60 < 80:
        st.subheader('Your intertemporal preferences moderately described by the {} Discounting model'.format(option))
    elif (Average(accuracy_list) * 100) < 60:
        st.subheader('Your intertemporal preferences poorly-described by the {} Discounting model'.format(option))
